/*
 * RTC client/driver for the Maxim/Dallas DS3232 Real-Time Clock
 *
...
#       define DS3232_REG_CR_RS2     0x10
#       define DS3232_REG_CR_RS1     0x08
...
int sqr_freq = 3; //0 off, 1 > 1.024Khz , 2 -> 4.096Khz , 3 -> 8.196Khz 
module_param(sqr_freq, int, 0);
...
static int ds3232_enable_square_wave_out(struct device *dev , uint8_t freq)
{
	...
	if (ret)
		return ret;
        control &= ~(DS3232_REG_CR_RS1 | DS3232_REG_CR_RS2);
	...
	switch(freq)
	{
		...
		case 1:
		{
	        	control |= DS3232_REG_CR_RS1;
		break;
		}
		case 2:
		{
	        	control |=  DS3232_REG_CR_RS2;
		break;
		}
		case 3:
		default:
...
/*
 * Temperature sensor support for ds3232 devices.
			...
			const char *name)
{
	...
	ds3232_enable_square_wave_out(dev, sqr_freq);
...
{
	return ds3232_enable_square_wave_out(dev, 0);
}
...
MODULE_DESCRIPTION("Maxim/Dallas DS3232 RTC Driver With Sqaure wave output");
MODULE_LICENSE("GPL");
